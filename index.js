...
// ⭐ MODIFIED: To accept and handle the optional 'size' field.
...
// ⭐ MODIFIED: To use size as a key identifier for finding the specific cart item.
...
// ⭐ NOTE: This route ONLY uses productId. If the user removes an item from the cart page,
// the frontend must send BOTH productId and size in the request body/params 
// OR the backend must be updated to handle removal by product+size. 
// For now, I'm assuming frontend will call a new endpoint for full removal.
...
// ## 4. Wishlist Management (No changes needed here as wishlist doesn't involve size)
// ...

// ## 6. Order History ----------------------------------------------------
// ⭐ MODIFIED: Ensure size is populated from the Order model's items
...
// POST /api/checkout
// ⭐ MODIFIED: To copy the 'size' field from the User's cart item into the new Order item.
